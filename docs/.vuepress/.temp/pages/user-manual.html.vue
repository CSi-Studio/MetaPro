...
/home/$USER/metapro/export
  ...
  <ol>
    ...
    <li>
      <p>
        ...
        For example:
        <code
          >纯文本 # Thermo file Original: file_name.raw Converted:
          file_name.aird, file_name.json # SCIEX file Original: file_name.wiff,
          file_name.wiff.scan Converted: file_name.aird, file_name.json
        </code>
      </p>
      <p>
        <strong>Step 2: File directory arrangement</strong> MetaPro enables
        efficient multi-batch files import by scanning folder structure under
        the previously set data repository (section 2.4). For example, if we set
        the data directory as <code>repository=/usr/local/data</code>
        The folder structure of different projects needs to be arranged as
        follows:
        <code
          >纯文本 /usr/local/data ├── project-1 ├── batch-1 ├──
          converted_file-1.aird ├── converted_file-1.json ├──
          converted_file-2.aird ├── converted_file-2.json ├── batch-2 ├──
          converted_file-3.aird ├── converted_file-3.json ├──
          converted_file-4.aird ├── converted_file-4.json ├── project-2 ├── ...
          ...
        </code>
  ...
  <ol>
    ...
    <li>
      ...
      <img src="/images/截屏2022-03-03 18.23.54.png" alt="" />
      If find no new file or the folder is empty, MetaPro will pop up the
      following prompt box:
      <img src="/images/截屏2022-03-03 18.22.44.png" alt="" />
      The file scanning progress can be find at the Task page. You can view the
      loaded files by <code>List → Samples</code> or single click on the project
      name. Sample list is presented as follows:
      <img src="/images/image_5.png" alt="" />
      You can view the files in different batches by selecting Batch name at the
      top of this page.
    </li>
  </ol>
  ...
  <ol>
    <li>
      ...
      interface. You can change the current overview to another by clicking on a
      specific overview. <img src="/images/image_16.png" alt="" />
    </li>
    <li>
      Compound switch module
      <ul>
        <li>
      ...
      <ul>
        <li>
          ...
          <code>Reset TargetRT</code> of a compound. Click on
          <code>Reset TargetRT</code>
          <img src="/images/image_18.png" alt="" />
    ...
    <li>
      Manual inspection module The buttons on the top are：<code>Restore</code>,
      <code>Select All/UnSelect</code>, <code>Manual Integration</code>,
      <code>Reselect</code>, <code>Submit</code>, <code>Batch Submit</code>
      ...
      batch/Cancel view all the peaks. <code>Manual Integration</code>: Enter
      manual integration mode on the selected peak. You can choose two data
      points (start, end) to modify the peak shape.
      <img src="/images/image_20.png" alt="" />
      <img src="/images/image_21.png" alt="" />
      <code>Reselect</code>: Click this to rechoose the data points.
      <code>Submit</code>: Submit the modified peak shape to database.
      <code>Batch Submit</code>: Apply the modified peak RT range to all samples
      in this batch. Used for integrating multiple peaks on an EIC. Click on
      <code>Manual Integration</code>, you can choose two data points to conduct
      manual integration functions. First left and then right.
    </li>
    <li>
      Spectrum viewer of <code>MS1</code> and <code>MS2</code>
      This is changed according to the sample you choose on the batch inspection
      module.
    ...
    <li>
      Peak shape batch inspection Users can choose multi-samples to change their
      status. Red represents for rejection. Yellow represents for unknown. Green
      represents for pass. The shortcuts are <code>1</code>, <code>2</code>,
      <code>3</code>. Keyboard <code>A</code> is to choose all the samples. If
      the status of a sample is labeled in yellow or red, its quantification
      result area will be set to 0. <img src="/images/image_24.png" alt="" />
    </li>
  </ol>
  ...
  <p>
    ...
    <code>1 second </code>. MetaPro accelerates repeated reading of files
    through caching technology. In the case of sufficient memory, MetaPro can
    load the file from disk only once when recalculating the first compound,
    save the file reading time for the inspection of other compounds, and
    achieve efficient batch manual quality control.
  </p>
  <p>
